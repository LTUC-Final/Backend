...
route.get("/DetailsOfCardInfo/:id", async (req, res) => {
    ...
    try {
        const dataCard = await pool.query(
            `SELECT s.product_id, s.provider_id,u.firstname,u.profile_image, c.category_id,c.name , c.description, s.name, s.location, s.description, s.price, s.type_of_product, s.image
        FROM products s
