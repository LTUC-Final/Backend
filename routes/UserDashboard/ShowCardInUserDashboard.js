...
route.get("/ShowCardInUserDashboard", async (req, res) => {
  ...
  try {
    ...
    const card = await pool.query(`SELECT s.product_id , p.provider_id ,u.firstname , u.lastname, s.category_id , s.name , s.location , s.price , s.image , g.name AS category_name,
